                ...
                if next_request is None:
                    if not queue_of_requests_to_retry.empty():
                        ...
                        try:
                            ...
                            data = json.loads(next(dataset))
                            # step 1:
                            # ask for reference
                            # if data['statement_source'] == 'other':
                            #     request_json["messages"][0]["content"] += \
                            #     data['statement_originator'] + " made this claim on " + data['statement_date'] + ". "
                            # # speech,television,news,blog,social_media,advertisement,
                            # # campaign,meeting,radio,email,testimony,statement
                            # else:
                            #     request_json["messages"][0]["content"] \
                            #         += "This claim is from " + data['statement_source'] \
                            #         + " on " + data['statement_date'] \
                            #         + " and " + data['statement_originator'] + " made the statement. "
                            # request_json["messages"][0]["content"] += "Claim:" + data['statement']

                            # climate/wice
                            # request_json["messages"][0]["content"] += "Claim:" + data['claim']

                            # pubHealth
                            # if not pd.isna(data['date_published']):  # is not NaN
                            #     request_json["messages"][0]["content"] += "This claim is published on " \
                            #                                               + data['date_published'] + ". "
                            # request_json["messages"][0]["content"] += "Claim:" + data['claim']

                            # step 2:
                            # classify the true or false label
                            # question = data[0]['messages'][0]['content']
                            # request_json["messages"][0]["content"] += question.split("Claim:")[1]
                            # request_json["messages"][0]["content"] += " Reference: "
                            # answer = data[1]['choices'][0]['message']['content']
                            # reference = answer.split("Reference: ")  # for invalid reference
                            # if len(reference) == 1:
                            #     request_json["messages"][0]["content"] += reference[0]
                            # else:
                            #     request_json["messages"][0]["content"] += reference[1]

                            # step 3:
                            # ask for multiple references
                            # climate/wice
                            # q = data[0]['messages'][0]['content']
                            # request_json["messages"][0]["content"] += "Claim: " \
                            #                                          + q.split("Claim:")[1].split('Reference:')[0]
